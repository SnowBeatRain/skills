...
涉及外键、触发器、视图或复杂约束时，不要只用“建新表 → 复制 → 删除旧表 → 重命名”。建议按 SQLite 官方 12-step 思路简化执行：

1. 迁移前记录旧表的索引、触发器、视图和外键依赖。
2. 如确需关闭外键检查，在事务开始前执行 `PRAGMA foreign_keys = OFF;`，并记录原状态。
3. 开启事务。
4. 创建新表。
5. 从旧表复制并转换数据。
6. 删除旧表。
...
10. 在同一事务内更新 `PRAGMA user_version` 或 `schema_migrations`。
11. 提交事务。
12. 恢复 `PRAGMA foreign_keys = ON;` 并查询确认。

如果 `foreign_key_check` 返回问题，必须回滚或阻断发布，不要推进版本号。
