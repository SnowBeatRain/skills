...
4. **读取最相关 reference**
   - 最小可改造模板：`references/minimal-plugin-template.md`
   ...
   - 调试/打包/发布排障：`references/debugging-and-release.md`
...
- [ ] 已确认 `uni.*` API / DCloud 插件不能满足需求。
- [ ] 已确认目标是 `APP-HARMONY`；若是 `MP-HARMONY`，先核实元服务能力边界，不默认套 ArkTS UTS 插件方案。
- [ ] 已查当前 HBuilderX、DevEco Studio、HarmonyOS SDK / API Level。
- [ ] 已选定 API 形态：单次异步、权限型、订阅型、原生组件型或 ohpm SDK 中转。
- [ ] 已设计 `Options`、`Success`、`Fail`、`Complete` 类型，并明确错误码。

发布或交付前必须验证：

- [ ] HBuilderX 运行到鸿蒙 App。
- [ ] 真机上验证成功路径、拒权路径、设备能力不可用路径。
- [ ] 本地打包或目标发布方式通过。
- [ ] 页面卸载后释放所有 listener/session/controller。
- [ ] 文档和日志不包含密钥、证书密码、token、真实隐私数据。

## 常用资源

- `references/minimal-plugin-template.md`：可复制改造的 `package.json`、`interface.uts`、`app-harmony/index.uts` 与示例页面骨架。
...
- `references/debugging-and-release.md`：HBuilderX、生成工程、ohpm、真机、日志、打包发布排障。
